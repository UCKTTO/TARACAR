...
public class CarouselView : MonoBehaviour {
    ...
    private IEnumerator InitUI() {
        ...
        image_width = view_window.rect.width/3;
        for (int i = 0; i < images.Length; i++)
        {
            images[i].sizeDelta = new Vector2(view_window.rect.width/3, view_window.rect.height);
            if (i >= 1) {
                images[i].anchoredPosition = new Vector2(((image_width + image_gap) * i), 0);
            }
        };
        image_gap = image_width * 0.25f;
    }
    ...
    void Update () {
        ...
        for (int i = 0; i < images.Length; i++)
        {
            images[i].sizeDelta = new Vector2(view_window.rect.width/3 * 0.75f, view_window.rect.height * 0.75f);
            images[i].anchoredPosition = new Vector2(screenPosition + ((image_width + image_gap) * i), 0);
        }
        images[current_index].sizeDelta = new Vector2(view_window.rect.width/3 * 1.75f, view_window.rect.height * 1.25f);
    }
    #endregion
